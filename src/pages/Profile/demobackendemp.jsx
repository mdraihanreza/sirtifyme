...
let websiteService = {
	...
	updateEmploymentDetails: async function (employmentData, authData, callback) {
		console.log('employmentData',employmentData); //return false;
		if(employmentData.c_job_name || employmentData.c_job_from_year)
		{
			...
			for (var i = 0; i < employmentData.empinfo.length; i++) {
				employment_data.push({
					user_id: authData.id,
					job_name: employmentData.empinfo[i].job_name,
					employment_start_date: employmentData.empinfo[i].employment_start_date,
					employment_end_date: employmentData.empinfo[i].employment_end_date
				});
			}
	...
	getEmploymentDetails: async function (authData, callback) {
		var user_employment_data = await UserEmployement.find({ "user_id":authData.id }).exec();
		console.log(user_employment_data,'user_employment_data')
	...
	getSubUserList: async function (user_id, authData, callback) {
		...
		if(sub_user_list.length > 0)
		{
			...
			//console.log(testimonial_arr);
			callback({ success: true, message: "sub user list fetched", data: sub_user_list_arr });
		}
		else
		{
			callback({ success: false, message: "No data found" });
	...
	deleteEmployment: async function (authData, employment_id, callback) {
		if(!employment_id) {
			callback({ success: false, message: "Employment id required" });
		}
		await UserEmployement.deleteOne({"_id": employment_id }).exec();
